...
export const parseOrganization = (organizations: string[]) => organizations.length ? '^' + organizations.map(escapeRegex).sort().join('|^') : '';
export const cutChannelIds = (idList: string[]) => idList.map(id => id.slice(0, 3) + id.slice(-3));
export const cutGroupString = (groupList: string) => groupList.split('|').map(group => group.slice(1, 3) + group.slice(-2));
